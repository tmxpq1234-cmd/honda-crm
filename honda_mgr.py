...
if st.session_state.user_name == "박스테반":
    with st.expander("⚙️ 팀장 전용 도구 (인사 및 업무 이전)"):
        ...
        with t1:
            ...
            with c2:
                del_n = st.selectbox("퇴사자 삭제", [u for u in user_db.keys() if u != "박스테반"])
                if st.button("삭제 실행"):
                    st.session_state.user_df = st.session_state.user_df[st.session_state.user_df['ID'] != del_n]
                    github_action(st.session_state.user_df, USER_FILE); st.rerun()
            ...
            with st.expander(f"📌 {row['고객명']} ({row['모델']}) | 인도일: {row['기준일']}"):
                ...
                for i, p in enumerate([1, 3, 6, 12]):
                    with cols[i]:
                        ...
                        m_txt = st.text_area("메시지", value=row.get(m_col, ""), key=f"m_{idx}_{p}", height=70)
                        if st.button("기록 저장", key=f"s_{idx}_{p}"):
