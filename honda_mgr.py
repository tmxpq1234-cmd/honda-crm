        ...
        with m_tab2:
            ...
            with u_col1:
                ...
                if st.button("✨ 등록", key="hr_reg_btn"):
                    user_df = pd.concat([user_df, pd.DataFrame([{"ID": new_n, "Password": "sonkim2024"}])], ignore_index=True)
        ...
        for idx, row in target_df.iterrows():
            ...
            with st.expander(f"📌 [{row['단계']}] {row['고객명']} ({row['모델']}) - 담당: {row['담당자']}"):
                if row['단계'] == "인도완료":
                    ...
                    if st.button("🚀 드라이브에 저장", key=f"up_btn_{prefix}_{customer_id}"):
                        if up_file:
                    ...
                    try:
                        ...
                        for i, p in enumerate([1, 3, 6, 12]):
                            ...
                            with cols[i]:
                                ...
                                m_txt = st.text_area("메모", value=row[m_col] if pd.notna(row[m_col]) else "", key=f"m_{prefix}_{p}_{customer_id}", height=70)
                ...
                note = st.text_area("비고", value=note_val, key=f"n_{prefix}_{customer_id}")
