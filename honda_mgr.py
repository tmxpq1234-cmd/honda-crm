...
st.markdown("""
    <style>
        ...
        /* 메인 타이틀 스타일 */
        .main-header { font-size: 30px; font-weight: 800; color: #1a1a1a; letter-spacing: -1px; margin-bottom: 20px; }
        
        /* 섹션 구분선 및 타이틀 */
        .section-title { font-size: 20px; font-weight: 700; color: #222; border-left: 6px solid #CC0000; padding-left: 12px; margin: 30px 0 15px 0; }
        ...
        /* 폼 및 데이터프레임 가독성 */
        .stForm { border-radius: 10px; border: 1px solid #eee !important; padding: 20px !important; }
...
if not st.session_state.logged_in:
    ...
    input_user = st.selectbox("사용자 선택", curator_list if curator_list else ["박스테반"])
...
# --- 6. 팀장 전용 도구 (박스테반 팀장 전용 기능 완벽 복구) ---
...
# --- 7. 고객 등록 및 리스트 (v7.6 레이아웃 100% 복구) ---
col_reg, col_view = st.columns([1, 3])

with col_reg:
    st.markdown('<p class="section-title">📍 신규 고객 등록</p>', unsafe_allow_html=True)
...
with col_view:
    t_all, t_con, t_del, t_can = st.tabs(["📊 전체", "📝 계약 현황", "🚚 인도 및 사후관리", "🚫 취소"])
    view_df = df.copy()
    ...
    def display_list(target_df, prefix):
        for idx, row in target_df.iterrows():
            ...
            with st.expander(f"📌 [{row['단계']}] {row['고객명']} ({row['모델']}) - 담당: {row['담당자']}"):
                # 드라이브 업로드 및 사후관리 (인도완료 전용 기능)
                if row['단계'] == "인도완료":
                    st.markdown("**📄 인도 서류 및 사후관리**")
                    up_file = st.file_uploader(f"서류 업로드 ({row['고객명']})", type=['jpg', 'pdf'], key=f"file_{prefix}_{cid}")
                    if st.button("🚀 드라이브 저장", key=f"up_btn_{prefix}_{cid}"):
                        if up_file:
                    ...
                    if pd.notna(row['비고']) and "http" in str(row['비고']):
                        st.link_button("📂 저장된 서류 보기", row['비고'], key=f"view_doc_{prefix}_{cid}")
                    ...
                    for i, p in enumerate([1, 3, 6, 12]):
                        ...
                        with cols[i]:
                            ...
                            m_txt = st.text_area("메시지 내용", value=row[m_col], key=f"m_{prefix}_{p}_{cid}", height=100)
                ...
                note = st.text_area("🗒️ 특이사항 및 비고", value=note_val, key=f"n_{prefix}_{cid}")
                ...
                if row['단계'] == "계약완료":
                    if c1.button("🚚 인도 완료 처리", key=f"d_{prefix}_{cid}"):
                        ...
                        save_data(df, CRM_FILE); st.rerun()
                    if c2.button("🚫 취소 처리", key=f"c_{prefix}_{cid}"):
