...
# --- 2. 디자인 설정 (Pretendard 서체) ---
st.set_page_config(page_title="HONDA CRM", layout="wide")
st.markdown("""
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Pretendard:wght@400;600;700&display=swap');
        html, body, [class*="css"] { font-family: 'Pretendard', sans-serif !important; }
        .main-header { font-size: 30px !important; font-weight: 700; color: #1a1a1a; margin-bottom: 5px; }
...
def github_action(df, path):
    if not GITHUB_TOKEN: return 401
...
with st.sidebar:
    st.write(f"🟢 **{st.session_state.user_name}** 팀장님 접속 중")
...
if st.session_state.user_name == "박스테반":
    with st.expander("⚙️ 팀장 전용 인사 및 인수인계"):
        t1, t2 = st.tabs(["👥 인사 관리", "🔄 업무 이전"])
        with t1:
            n_n = st.text_input("신입 성함"); n_p = st.text_input("비밀번호", "2290")
            if st.button("큐레이터 등록"):
                ...
                github_action(st.session_state.user_df, USER_FILE); st.success("등록 완료"); st.rerun()
        with t2:
            src = st.selectbox("기존 담당", list(user_db.keys())); tgt = st.selectbox("인수자", [u for u in user_db.keys() if u != src])
            if st.button("고객 데이터 일괄 인수인계"):
    ...
    with st.form("reg_form", clear_on_submit=True):
        ...
        if st.form_submit_button("시스템 저장"):
            if name:
                ...
                github_action(st.session_state.crm_df, FILE_PATH); st.success(f"{name}님 저장 완료!"); st.rerun()

with c_view:
    tab_con, tab_ind, tab_care = st.tabs(["📝 계약 현황", "🚚 인도 완료 현황", "📅 인도 고객 사후관리"])
    df = st.session_state.crm_df
    v_df = df if st.session_state.user_name == "박스테반" else df[df['담당자'] == st.session_state.user_name]

    with tab_con: # 계약 현황
        st.dataframe(v_df[v_df['단계'] == "계약완료"], use_container_width=True)
    
    with tab_ind: # 인도 완료 현황
        st.dataframe(v_df[v_df['단계'] == "인도완료"], use_container_width=True)

    with tab_care: # 사후관리 (메시지 및 비고)
        care_df = v_df[v_df['단계'] == "인도완료"]
        if care_df.empty: st.info("인도 완료된 고객이 없습니다.")
        for idx, row in care_df.iterrows():
            with st.expander(f"📌 {row['고객명']}님 ({row['모델']}) | 인도일: {row['기준일']}"):
                # 사후관리 스케줄러 (1, 3, 6, 12개월)
                ...
                for i, p in enumerate([1, 3, 6, 12]):
                    with cols[i]:
                        ...
                        is_s = st.checkbox("발송완료", value=bool(row.get(s_col, 0)), key=f"c_{idx}_{p}")
                        m_txt = st.text_area("메시지 내용", value=row.get(m_col, ""), key=f"t_{idx}_{p}", height=80)
                ...
                st.divider()
                # 통합 비고란
                note = st.text_area("🗒️ 고객 통합 비고 (상담 내용 등)", value=row.get('비고', ""), key=f"note_{idx}")
                if st.button("비고 최종 저장", key=f"nb_{idx}"):
                    st.session_state.crm_df.at[idx, '비고'] = note
                    github_action(st.session_state.crm_df, FILE_PATH); st.success("비고 저장됨"); st.rerun()
